//
// Created by tati2327 on 14/04/19.
//

#include "Pathfinding.h"

bool Pathfinding::isValid(int x, int y) { //If our Node is an obstacle it is not valid
...
bool Pathfinding::isDestination(int x, int y, Node dest) {
...
double Pathfinding::calculateH(int x, int y, Node dest) {
    double H = (sqrt((x - dest.x)*(x - dest.x)
                     + (y - dest.y)*(y - dest.y)));
    return H;
}

vector<Node> Pathfinding::makePath(array<array<Node, (Y_MAX / Y_STEP)>, (X_MAX / X_STEP)> map, Node dest) {
    try {
...
vector<Node> Pathfinding::aStar(Node player, Node dest) {
    vector<Node> empty;
